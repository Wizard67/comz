...
describe('useBEM', () => {
  it('should return a BEM style class name.', () => {
    expect(useBEM(({b, e, m}) => ({
      ...
      [e('element')]: true,
      [m('modifier')]: true
    })).value).toBe('block__element block__element--modifier')
  
    useBEM(({b, e, m}) => ({
      [e('block')]: true,
      [m('modifier')]: true
    })).value
    expect(`useBEN need to declare a block element with b().`).toHaveBeenWarned()

    expect(useBEM(({b, e, m}) => ({
      [b('block')]: true,
      [m('modifier1')]: true,
      [m('modifier2')]: true
    })).value).toBe('block block--modifier1 block--modifier2')

    expect(useBEM(({b, e, m}) => ({
      [b('block')]: true,
      [e('element1')]: true,
      [e('element2')]: true
    })).value).toBe('block__element2')
